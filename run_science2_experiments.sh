...
# Science2 phase: larger-scale, seeded experiments with warmups and eval summaries
# - Reuses composite env SPECS from science suite
# - Three experiments per seed:
#   1) baseline_nomask: no masking, no zeroing (graded formatting)
#   2) softmask_every10_wt05: mask correctness every 10th correct with 0.5 weight (+warmup)
#   3) fullzero_every10_nothresh: zero every 10th fully-correct ignoring format (set format_full_threshold=0.0) (+warmup)

OUTDIR_BASE="exp_output/science2_suite"
mkdir -p "$OUTDIR_BASE"
MASTER_LOG="${OUTDIR_BASE}/batch_science2_launch.log"
echo "==== Science2 suite launch started $(date) ====" | tee -a "$MASTER_LOG"
...
WANDB_PROJECT=${WANDB_PROJECT:-nano-grpo-sciencefullset}
# Alternate project for continuous add-on experiments
...
# Light softmask defaults (gentler variant to run last)
LIGHT_EVERY_N=${LIGHT_EVERY_N:-20}
LIGHT_WEIGHT=${LIGHT_WEIGHT:-0.7}
LIGHT_MASK_WARMUP_STEPS=${LIGHT_MASK_WARMUP_STEPS:-400}

# Seeds for statistical reliability
SEEDS=(${SEEDS:-1 2})
# Single-seed for continuous add-on experiments
...
softmask_every10_wt05        | --reward_mask_strategy every_n --reward_mask_every_n 10 --reward_mask_weight 0.5 --correctness_weight 1.0 --format_weight 1.0 --full_correct_zero_strategy none --mask_warmup_steps ${MASK_WARMUP_STEPS}
fullzero_every10_nothresh    | --reward_mask_strategy none --correctness_weight 1.0 --format_weight 1.0 --full_correct_zero_strategy every_n --full_correct_zero_every_n 10 --format_full_threshold 0.0 --zero_warmup_steps ${ZERO_WARMUP_STEPS}
EXPS
)

for SEED in "${SEEDS[@]}"; do
  echo "[INFO] Launching experiments for seed=${SEED}" | tee -a "$MASTER_LOG"
  while IFS='|' read -r NAME ARGS; do
    ...
    # Disable TorchDynamo broadly for stability with fused loss; override per-env if needed
    PER_EXP_ENV="TORCHDYNAMO_DISABLE=${TORCHDYNAMO_DISABLE:-1}"

    # Build checkpoint arg controls
    EXTRA_SAVE_ARGS=""
    ...
    # Choose W&B project based on experiment name (cont_* goes to WANDB_PROJECT_CONT)
...
echo "==== Science2 suite launch finished $(date) ====" | tee -a "$MASTER_LOG"
echo "Main suite done. Inspect logs and summaries under $OUTDIR_BASE" | tee -a "$MASTER_LOG"

# # ---------------- Continuous add-on block (runs after main suite) ----------------
...
# echo "==== Science2 continuous add-on launch started $(date) ====" | tee -a "$CONT_MASTER_LOG"
...
# echo "==== Science2 continuous add-on launch finished $(date) ====" | tee -a "$CONT_MASTER_LOG"
